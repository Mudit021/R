...
# We create a separate dataframe here to remove rows where 'Age' is NA for age-specific analysis.
...
fare_outliers <- titanic_data %>% filter(Fare < lower_bound_fare | Fare > upper_bound_fare)
outlier_count_fare <- nrow(fare_outliers)
...
cat("Upper Bound for Outliers:", upper_bound_fare, "\n")
cat("Number of passengers with fares outside the 1.5 * IQR range:", outlier_count_fare, "\n\n")


# --- Step 9: Calculate, Count, and Cap Age Outliers ---
# We use the 'titanic_age_data' which has NA ages removed.
Q1_age <- quantile(titanic_age_data$Age, 0.25)
Q3_age <- quantile(titanic_age_data$Age, 0.75)
IQR_age <- IQR(titanic_age_data$Age)
lower_bound_age <- Q1_age - 1.5 * IQR_age
upper_bound_age <- Q3_age + 1.5 * IQR_age
age_outliers <- titanic_age_data %>% filter(Age < lower_bound_age | Age > upper_bound_age)
outlier_count_age <- nrow(age_outliers)

cat("\n--- Titanic Age Outlier Analysis ---\n")
cat("First Quartile (Q1) for Age:", Q1_age, "\n")
cat("Third Quartile (Q3) for Age:", Q3_age, "\n")
cat("Interquartile Range (IQR) for Age:", IQR_age, "\n")
cat("Lower Bound for Outliers:", lower_bound_age, "\n")
cat("Upper Bound for Outliers:", upper_bound_age, "\n")
cat("Number of passengers with ages outside the 1.5 * IQR range:", outlier_count_age, "\n\n")

# Capping the outliers: Replace values outside the bounds with the boundary values.
titanic_data_capped <- titanic_age_data %>%
  mutate(Age_capped = ifelse(Age > upper_bound_age, upper_bound_age,
                        ifelse(Age < lower_bound_age, lower_bound_age, Age)))

cat("Outliers in the 'Age' column have been replaced (capped).\n")
cat("Summary of original 'Age' column:\n")
print(summary(titanic_age_data$Age))
cat("\nSummary of new 'Age_capped' column (note the new min and max):\n")
print(summary(titanic_data_capped$Age_capped))
cat("\n\n")


# --- Step 10: Final Message ---
cat("Titanic analysis complete. All plots and calculations have been generated and saved.\n")